...
date: "Created: 2024-06-13, last modified: 2024-06-13, compiled: `r Sys.Date()`"
abstract: "We ... and illustrate how to invoke them using the novel **pnd** package for **R**. Finally, we discuss the real-world limitations of numerical derivatives and offer empirically backed advice for common situations where finite-difference approximations are unreliable."
...
As of 2024, most popular R numerical optimisers and functions for computing numerical derivative approximations continue to rely on serial evaluation of functions, and even in scenarios where parallelisation is feasible, users often lack a convenient parallel solution.
...
This article provides algebraic derivations of the optimal step size for numerical differentiation routines, simplified recommendations, and illustrated simulations for common functions.
...
If $f(x)$ is continuous on $[\underline{x}, \overline{x}]$, $(x_1,\ldots, x_n)$ are points belonging to $[\underline{x}, \overline{x}]$, and $p_1, \ldots, p_n > 0$ is a vector of positive numbers, then, $\exists x^* \in [\underline{x}, \overline{x}]$ such that $f(x^*) \cdot \sum_i p_i = \sum_i p_i f(x_i)$.
(@sauer2017numerical provides a proof on p. 256.)
...
\textcolor{red}{!!! TODO: Check notation in the document: should subtract $b_i$? Homogenise}

\textcolor{red}{!!! Check the definition of the machine epsilon everywhere -- ensure that the error bound is macheps/2}
...
The solution for the optimal weights $(w_1, \ldots, w_n)$ in Equation~\eqref{eq:wsum} are due to @taylor2016finite: for a stencil $(b_1, \ldots, b_{n})$, and for derivative order $m$, the weights that zero out the sums of derivatives of orders other than $m$ and result in a unit coefficient on the $m$\textsuperscript{th} derivative must satisfy
\[
\begin{pmatrix}
b_1^0  & \cdots  & b_n^0 \\
\vdots & \ddots & \vdots  \\
...
\textcolor{red}{!!! Write an example: computing $f''(-3)$ if there are values $f(1, 2, 4, 5)$. Stencil: $(4, 5, 7, 8)$}

\textcolor{red}{!!! Check the notation: $f_{CD}(x, h)$ must contain h everywhere!}

The function `fdCoef(...)` from `pnd` replicates the functionality of the popular online calculator by @taylor2016finite, equivalent to the iterative algorithm in @fornberg1988generation.
...
Our implementation solves the corresponding system of linear equations via the LAPACK routine invokable through `base::solve()`.
...
\textcolor{red}{TODO: return the actual order, i.e. the first non-zero term}
...
\textcolor{red}{!!! Move the details to a different section (one-sided), keep the idea and refer to that section}
...
\textcolor{red}{!!! Describe how using the argument `precomputed` and `f0` can save time}
...
\textcolor{red}{!!! Beyond this point, the material was typed up once and not proof-read / simplified}
...
\[
\varepsilon'_3(h) = \frac{17|f^{(V)}(x)|}{30}h - 4.5|f(x)|\epsilon_{\mathrm{m}} h^{-4} = 0,
\]
\[
\varepsilon'_4(h) = \frac{17|f^{(VI)}(x)|}{45}h - 32|f(x)|\epsilon_{\mathrm{m}} h^{-5} = 0,
\]
...
Gauging the truncation error of the off-diagonal elements, $H_{ij}$ for $i\ne j$, requires the multi-variate Taylor expansion:
\[
...
Denote the bivariate step vector $h_{ij} := (h_i + h_j)$.
The first numerator term of the Hessian finite-difference approximation becomes
\[
...
Since only 2 elements of $h_{ij}$  are non-zero, most elements of the triple sum are zero:
\[
\sum_{k_1 \ne k_2 \ne k_3}  \frac{\partial^3 f(x)}{\partial x_{k_1} \partial x_{k_2} \partial x_{k_3} }  \underbrace{h_{ij}^{({k_1})} h_{ij}^{(k_2)} h_{ij}^{(k_3)}}_{=0} = 0.
\]
The only non-zero terms affecting the error are the elements with indices $k_1, k_2, k_3 \in \{i, j\}^3$.
There are 8~such combinations: $(i,i,i), (i,i,j), \ldots, (j,j,i), (j,j,j)$.
In all these combinations, $h_{ij}^{({k_1})} h_{ij}^{(k_2)} h_{ij}^{(k_3)} = h^3$ because the only non-zero elements of $h_{ij}$ in positions $i$ and $j$ are equal to $h$.
Denote third derivatives by  $f'''_{ijk} := \frac{\partial^3 f}{\partial x_i \partial x_j \partial x_k}$ -- then the triple sum becomes $h^3 \sum_{k_1, k_2, k_3 \in \{i,j\}^3}(f'''_{k_1 k_2 k_3})$.

Expansions of $f(x - h_i + h_j)$, $f(x + h_i - h_j)$,  and $f(x - h_i - h_j)$ are similar, with the only difference in signs of $(f'_i, f'_j)$, $(f''_{ii}, f''_{ij}, f''_{ji}, f''_{jj})$, and $(f'''_{iii}, f'''_{iij}, \ldots, f'''_{jjj})$: if $h_i$ is subtracted, then, wherever the derivative w.r.t. the $i$^th^ coordinate is taken an odd number of times, the sign of the coefficient on the term flips.
The same argument applies to the sign change due to subtracting $h_j$.
...
Computing the expression for the numerator of $H_{ij}$ requires adding up the Taylor expansion terms with the signs from each row of this table; in rows 2 and 3 all the signs should be flipped because these terms of $\hat H_{ij}$ have a negative sign on them.
...
\textcolor{red}{!!! Add other rules here}
